...
NntpArticle::NntpArticle(
        NntpFile *file, uint part, qint64 pos, qint64 bytes, std::string const *from, bool obfuscation)
    : _nntpFile(file)
    , _part(part)
...
void NntpArticle::write(NntpConnection *con, std::string const &idSignature)
{
    ++_nbTrySending;
...
std::string NntpArticle::header(std::string const &idSignature) const
{
#if QT_VERSION >= QT_VERSION_CHECK(5, 11, 0)
...
void NntpArticle::dumpToFile(QString const &path, std::string const &articleIdSignature)
{
